...
extern "C" int serialClose(int64_t handle_ptr, ErrorCallbackT error_callback)
{
    ...
    {
        invokeError(std::to_underlying(cpp_core::StatusCodes::kSetStateError), "serialClose: Failed to restore attributes", error_callback);
        // Continue with close anyway.
    }

    if (close(handle->fd) != 0)
    {
        invokeError(std::to_underlying(cpp_core::StatusCodes::kCloseHandleError), "serialClose: Failed to close port", error_callback);
        delete handle;
